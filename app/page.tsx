...
const ConfigPage = () => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    const fullUrl = `${currentUrl}/api/config?proxy=${encodeURIComponent(
      proxy
    )}`;
    setUrl(fullUrl);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full space-y-4">
        <div>
          <a href="/api/mmdb" className="text-indigo-600 hover:underline">
      ...
      <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full space-y-4">
        <div>
          <a href="/api/wintun" className="text-indigo-600 hover:underline">
            Download wintun.dll
          </a>
        </div>
      </div>
